...
class Z extends d {
  ...
  #S() {
    ...
    // 使用 1.0 作为 decay (线性衰减)，使光源辐射范围更大、光照强度更高，在鼠标所在处形成明显的光晕与高光
    this.light = new u(this.config.colors[0], this.config.lightIntensity, 0, 1.0);
    this.add(this.light);
  }
  setColors(e) {
...
function createBallpit(e, t = {}) {
  ...
  // 1. 先注册 onAfterResize 回调，以便在 resize() 执行时能够动态更新 maxX/maxY
  i.onAfterResize = e => {
    if (s) {
  ...
  // 2. 执行首次测量，此步骤会计算出真实的 i.size.wWidth 和 i.size.wHeight
  i.resize();

  // 3. 将计算出的真实边界融入初始配置中，确保物理网格创建时球体能够散落分布在整个屏幕空间
  const initialConfig = {
    ...t,
  ...
  function initialize(e) {
    ...
    // 始终保证物理引擎边界是以当前窗口测量的 wWidth/wHeight 为准，防止被 React 的默认 Props (5) 覆写
    const merged = {
      ...e,
      maxX: i.size.wWidth / 2,
      maxY: i.size.wHeight / 2
    };
    s = new Z(i.renderer, merged);
    i.scene.add(s);
  }
  i.onBeforeRender = e => {
    if (!c) s.update(e);
  ...
  return {
    ...
    updateConfig(newConfig) {
      if (!s) return;
      // 始终将边界锁定为当前实际测量大小，防止被 React 组件 Props 中的默认值 (5) 覆写
      const finalConfig = {
        ...newConfig,
        maxX: i.size.wWidth / 2,
        maxY: i.size.wHeight / 2
      };
      const countChanged = finalConfig.count !== undefined && finalConfig.count !== s.config.count;
      const colorsChanged = finalConfig.colors !== undefined && JSON.stringify(finalConfig.colors) !== JSON.stringify(s.config.colors);
      if (countChanged || colorsChanged) {
        initialize(finalConfig);
      } else {
        Object.assign(s.config, finalConfig);
        Object.assign(s.physics.config, finalConfig);
        if (finalConfig.minSize !== undefined || finalConfig.maxSize !== undefined || finalConfig.size0 !== undefined) {
          s.physics.setSizes();
        }
        if (finalConfig.ambientColor !== undefined) {
          s.ambientLight.color.set(finalConfig.ambientColor);
        }
        if (finalConfig.ambientIntensity !== undefined) {
          s.ambientLight.intensity = finalConfig.ambientIntensity;
        }
        if (finalConfig.lightIntensity !== undefined) {
          s.light.intensity = finalConfig.lightIntensity;
        }
      }
    },
    dispose() {
